...
export function TasksList() {
    ...
    return (
        <div className="tasks-list-wrapper">
            { /*tasks.filter(task => task.status === TaskStatus.NEW).map(task => <TaskCard key={task.id} task={task} onTaskDelete={onDeleteHandler} changeStatus={onChangeStatusHandler}/>)*/}
            ...
            <div className="status in-review" onDragOver={(event) => event.preventDefault()} onDrop={(event) => onDropHandler(event, TaskStatus.IN_REVIEW)}>
                <div className="column-header">InReview</div>
